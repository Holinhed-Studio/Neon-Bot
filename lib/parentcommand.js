...
class ParentCommand {
   ...
   load(api) {
      this.acknowledge_invalid = api.settingsManager.getGlobal().commands.parentCommand.acknowledge_invalid;
      ...
      try {
         ...
         console.log(colors.format(colors.bg.red, colors.fg.white),`[CRITICAL] Could not load command directory '${this.name}'. (Error below)`);
         console.log(err);
         console.log(colors.format(colors.bg.red, colors.fg.white), '^^^ END ERROR ^^^');
         return;
      }
   ...
   payload(api, args) {
      ...
      try {
         if (!this.commandMap[cmd]) {
            if (this.acknowledge_invalid) {
               api.message.channel.send(`Command ${this.name != 'commands' ? `${this.title}::` : ''}${cmd} not found.`);
            }
            return;
         }
         ...
         const result = this.commandMap[cmd].payload({...api, ['parentcmd']: this}, pargs);

      ...
      files.forEach(file => {
         ...
         try {
            ...
            if (command instanceof ParentCommand) {
               ...
               command.parents.push(this.name);
               command.load();
            }

            if (!command.parents) {
               command.parents = [...this.parents, this.name];
            }
   ...
   payload(api, args) {
      ...
      try {
         ...
         if (result === 2) {
            ...
            const hist = this.commandMap[cmd].parents.join(' ');
            api.message.channel.send(`Usage: ${prefix}${hist} ${cmd} ${this.commandMap[cmd].usage || ''}`);
         }
